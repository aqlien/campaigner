...
describe Response, type: :model do
  ...
  describe 'applicable_attributes' do
    ...
    it 'should not have string_value if response_type is answer' do
      ...
      expect(Response.applicable_attributes(bad)).to eq('question_id' => question.id, 'answer_id' => answer.id)
    end
    ...
    it 'should have string_value if response_type is string and answer_id is an array (in the case of checkboxes)' do
      ...
      expect(Response.applicable_attributes(good)).to eq('question_id' => question.id, 'answer_id' => ['', answer.id])
    end

    it 'should have ignore attribute if missing answer_id' do
      ignore = { 'question_id' => question.id, 'answer_id' => '', 'string_value' => 'Frank' }
      expect(Response.applicable_attributes(ignore)).to eq('question_id' => question.id, 'answer_id' => '', 'string_value' => 'Frank')
    end
