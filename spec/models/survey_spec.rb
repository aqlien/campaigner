...
RSpec.describe Survey, type: :model do
  ...
  it { expect(Survey.new(event: Event.new(), title: 'Survey').valid?).to be true }
  ...
  context 'with survey_sections' do
    ...
    it 'deletes child survey_sections when deleted' do
      ...
      survey_section_ids.each do |id|
        expect(SurveySection.find_by_id(id)).to be_nil
      end
    end
  end
