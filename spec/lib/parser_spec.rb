...
describe Parser do
  it 'should return a survey object' do
    expect(Parser.new.parse("survey 'hi' do\nend").is_a?(Survey)).to eq(true)
  end

  context 'basic questions' do
    let(:file) { File.new(TEST_SURVEYS_PATH + '/favorites.rb') }
    subject! { Parser.new.parse(File.read(file)) }
  ...
  context 'complex questions' do
    let(:file) { File.new(TEST_SURVEYS_PATH + '/feelings.rb') }
    subject! { Parser.new.parse(File.read(file)) }
  ...
  context 'translations' do
    it 'should produce the survey text for :default locale' do
      ...
      survey = Parser.new.parse(survey_text)
    ...
    it 'should raise an error w/o :default locale' do
      ...
      s = Survey.all.size
      expect { Parser.new.parse(survey_text) }.to raise_error(ParserError)
        ...
        it 'should look for the translation file relative to pwd' do
          Dir.mktmpdir do |dir|
            FileUtils.cd(dir) do
              ...
              Parser.parse(File.read(survey_temp_file.path))
      ...
      context 'when the survey filename is given' do
        it 'should look for the translation file relative to the survey directory' do
          Dir.mktmpdir do |dir|
            ...
            survey_temp_file.write(survey_text)
            survey_temp_file.flush
            Parser.parse_file(survey_temp_file.path)
            survey = Survey.where(title: 'One language is never enough').first
            expect(survey.nil?).to eq(false)
            expect(survey.translation(:es)['title']).to eq('Un idioma nunca es suficiente')
          end
  ...
  context 'quizzes' do
    let(:file) { File.new(TEST_SURVEYS_PATH + '/numbers.rb') }
    subject! { Parser.new.parse(File.read(file)) }
  ...
  context 'failures' do
    it 'typos in blocks' do
      ...
      '
      expect { Parser.parse(survey_text) }.to raise_error(ParserError, /\"sectionals\" is not a surveyor method./)
    end
    ...
    it 'bad references' do
      ...
      '
      expect { Parser.parse(survey_text) }.to raise_error(ParserError, /Bad references: q_1; q_1, a_1; q_watch, a_b_1/)
    end
    ...
    it 'repeated references' do
      ...
      '
      expect { Parser.parse(survey_text) }.to raise_error(ParserError, /Duplicate references: q_watch, a_1; q_watch/)
    end
    it 'with Rails validation errors' do
      ...
      '
      expect { Parser.parse(survey_text) }.to raise_error(ParserError, /Survey not saved: Title can't be blank/)
    end
    ...
    it 'bad shortcuts' do
      ...
      '
      expect { Parser.parse(survey_text) }.to raise_error(ParserError, /\"quack\" is not a surveyor method./)
    end
  end
