module SurveysHelper
  ...
  def previous_section
    ...
    submit_tag(t('surveys.previous_section').html_safe, :name => "section[#{@sections[prev_index].id}]") unless @sections[0] == @section
  end
  def next_section
    # use copy in memory instead of making extra db calls
    next_index = [(@sections.index(@section) || @sections.count) + 1, @sections.count].min
    @sections.last == @section ? submit_tag(t('surveys.click_here_to_finish').html_safe, :name => "finish") : submit_tag(t('surveys.next_section').html_safe, :name => "section[#{@sections[next_index].id}]")
  end
